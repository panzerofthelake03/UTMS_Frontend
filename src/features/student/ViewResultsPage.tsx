...
export default function ViewResultsPage() {
  ...
  async function handleDownloadLetter() {
    ...
    setDocError(null);
    try {
      // Try to find a server-side acceptance letter first
      ...
      // Fall back: generate acceptance letter as HTML and open in new tab for printing/saving
      const html = generateAcceptanceLetter(app);
      ...
      if (win) {
        ...
        // pop-up blocked → download as file
        const link = document.createElement('a');
        link.href = url; link.download = `acceptance-letter-${app.id}.html`;
  ...
  function generateAcceptanceLetter(application: NonNullable<typeof app>): string {
    const date = new Date().toLocaleDateString('en-GB', { day: '2-digit', month: 'long', year: 'numeric' });
    const term = application.term.replace('-', '–');
    const dept = application.targetDepartment ?? 'Computer Engineering';
    return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8"/>
  <title>Acceptance Letter — UTM-2025-${application.id}</title>
  <style>
    body { font-family: 'Times New Roman', Times, serif; max-width: 700px; margin: 60px auto; color: #111; }
    .header { text-align: center; border-bottom: 3px solid #8b1a1a; padding-bottom: 16px; margin-bottom: 32px; }
    .header h1 { margin: 0; font-size: 20px; color: #8b1a1a; }
    .header p { margin: 4px 0 0; font-size: 13px; color: #555; }
    h2 { font-size: 17px; text-align: center; margin-bottom: 24px; text-transform: uppercase; letter-spacing: 1px; }
    p { line-height: 1.8; font-size: 14px; }
    .meta { margin-top: 32px; font-size: 13px; color: #555; border-top: 1px solid #ddd; padding-top: 12px; }
...
<body>
  <div class="header">
    <h1>İzmir Institute of Technology (IYTE)</h1>
    <p>Undergraduate Transfer Management System (UTMS)</p>
  </div>
  <h2>Letter of Acceptance</h2>
  <p>Date: ${date}</p>
  <p>Reference No: UTM-2025-${application.id}</p>
  <br/>
  <p>Dear Student,</p>
  <p>
    We are pleased to inform you that your application for undergraduate transfer to
    <strong>İzmir Institute of Technology (IYTE)</strong> has been reviewed and
    <strong>accepted</strong> for the <strong>${term}</strong> academic term.
  </p>
  <p>
    You have been admitted to the <strong>${dept}</strong> department.
    Please report to the Student Affairs Office (ÖİDB) during the registration period
    (September 20–25) with the following original documents:
  </p>
  <ul>
    <li>Original identity document (National ID or Passport)</li>
    <li>Official transcript from your previous institution</li>
    <li>This acceptance letter (printed or digital)</li>
  </ul>
  <p>
    Failure to complete registration within the stated period may result in
    cancellation of your placement.
  </p>
  <p>We look forward to welcoming you to IYTE.</p>
  <div class="signature">
    <p>Sincerely,</p>
    <br/><br/>
    <p><strong>Student Affairs Office (ÖİDB)</strong></p>
    <p>İzmir Institute of Technology</p>
    <p>Gülbahçe Kampüsü, 35430 Urla/İzmir, Turkey</p>
  </div>
  <div class="meta">
    Application ID: UTM-2025-${application.id} •
    Generated: ${new Date().toLocaleString('en-GB')} •
    This document is digitally generated by UTMS.
  </div>
